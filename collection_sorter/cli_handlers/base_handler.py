...
class CommandHandler(ABC):
    ...
    def prepare_destination(self) -> Result[Optional[FilePath], OperationError]:
        ...
        try:
            destination_path = FilePath(
                self.destination,
                must_exist=False,
                create_if_missing=not self.dry_run
            )
    ...
    def handle_result(
        self,
        result: Result[Dict[str, Any], List[OperationError]]
    ) -> Dict[str, Any]:
        """
        ...
        if result.is_success():
            ...
            console.print(f"[green]Successfully processed {sources_processed} sources[/green]")
...
class TemplateMethodCommandHandler(CommandHandler):
    ...
    def handle(self) -> Result[Dict[str, Any], List[OperationError]]:
        ...
        try:
            ...
            postprocess_result = self.post_process(source_paths, destination_path, processed_items)
