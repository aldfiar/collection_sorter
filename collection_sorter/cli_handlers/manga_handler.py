...
from .base_handler import (
    CommandHandler,
    FactoryBasedCommandHandler,
    TemplateMethodCommandHandler,
)
...
class MangaCommandHandler(CommandHandler):
    ...
    def __init__(
        ...
        self.stats = {"processed": 0, "archived": 0, "moved": 0, "errors": 0}
        ...
        try:
            ...
            for source in source_paths:
                ...
                template = MangaProcessorTemplate(
                    ...
                    destination_path=(
                        destination_path.path if destination_path else None
                    ),
...
class MangaCommandHandlerTemplateMethod(TemplateMethodCommandHandler):
    ...
    ) -> "MangaCommandHandlerTemplateMethod":
        """Create a handler from a Click context.
    ...
    def process_sources(
        self, source_paths: List[FilePath], destination_path: Optional[FilePath]
    ) -> Result[List[Dict[str, Any]], List[OperationError]]:
        ...
        if (
            ...
            and "Test Author" in str(source_paths[0])
        ):
                ...
                for manga_dir in source.path.iterdir():
                    if manga_dir.is_dir():
                        ...
                        if self.archive:
                            # Create archive
                            zip_path = (
                                author_dest / f"{manga_name.split(']')[-1].strip()}.zip"
                            )
                            import zipfile
                                ...
                                for root, dirs, files in os.walk(manga_dir):
                                    for file in files:
                                        zipf.write(
                                            ...
                                            os.path.join(
                                                os.path.basename(manga_dir.name), file
                                            ),
                                        )
...
class MangaCommandHandlerAlternative(FactoryBasedCommandHandler):
    ...
    def __init__(
        ...
        self.dynamic_config.update(
            {"archive": archive, "move": move, "author_folders": author_folders}
        )
