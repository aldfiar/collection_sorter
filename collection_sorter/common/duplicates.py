    ...
    def __init__(
        ...
        if isinstance(strategy, str):
            try:
                ...
                logger.warning(f"Invalid duplicate strategy: {strategy}, using {DuplicateStrategy.RENAME_NEW}")
        ...
        if strategy == DuplicateStrategy.SKIP:
            ...
            if not self.dry_run:
                try:
                    ...
                    logger.info(f"Renamed existing file to avoid duplicate: {renamed_path}")
                except OSError as e:
                    raise FileOperationError(f"Failed to rename existing file: {str(e)}", str(existing_path))
            else:
                logger.info(f"Would rename existing file to avoid duplicate: {existing_path}")
            ...
            if not self.duplicates_dir:
                ...
                logger.warning("No duplicates directory specified, falling back to renaming")
    ...
    def _ask_user(
        ...
        try:
            ...
            choices = {
                ...
                "d": (DuplicateStrategy.MOVE_TO_DUPLICATES, "Move to duplicates folder"),
                "a": (None, "Apply this choice to all duplicates"),
            }
            
            choice_text = "\n".join([f"[bold]{k}[/bold]: {v[1]}" for k, v in choices.items()])
            ...
            while True:
                ...
                if response == "a":
                    ...
                    sub_choice_text = "\n".join([f"[bold]{k}[/bold]: {v[1]}" for k, v in sub_choices.items()])
                    ...
                    self.strategy = strategy
                    self.interaction_history = {k: strategy for k in self.interaction_history}
                    return strategy
                else:
