...
class StrategyFactory(Factory[S]):
    ...
    def __init__(
        self, default_dry_run: bool = False, default_compression_level: int = 6
    ):
        """
    ...
    def create(
        ...
        **kwargs,
        ...
        compression_level = (
            ...
            else self.default_compression_level
        )

        # Create the appropriate strategy
        if strategy_type == "move_file":
            return MoveFileStrategy(
                dry_run=dry_run, duplicate_handler=duplicate_handler
            )
        elif strategy_type == "copy_file":
            return CopyFileStrategy(
                dry_run=dry_run, duplicate_handler=duplicate_handler
            )
            ...
            return ExtractArchiveStrategy(
                dry_run=dry_run, duplicate_handler=duplicate_handler
            )
        elif strategy_type == "rename_file":
            return RenameFileStrategy(
                dry_run=dry_run, duplicate_handler=duplicate_handler
            )
        else:
...
class ResultStrategyFactory(Factory[RS]):
    ...
    def create(
        ...
        **kwargs,
        ...
        # Create the appropriate strategy
        if strategy_type == "move_file":
            return MoveFileResultStrategy(
                dry_run=dry_run, duplicate_handler=duplicate_handler
            )
        elif strategy_type == "copy_file":
            return CopyFileResultStrategy(
                dry_run=dry_run, duplicate_handler=duplicate_handler
            )
        elif strategy_type == "archive":
            ...
            return ExtractArchiveResultStrategy(
                dry_run=dry_run, duplicate_handler=duplicate_handler
            )
        elif strategy_type == "rename_file":
            return RenameFileResultStrategy(
                dry_run=dry_run, duplicate_handler=duplicate_handler
            )
...
class ProcessorFactory(Factory[P]):
    ...
    def __init__(
        ...
        self.duplicate_handler_factory = (
            duplicate_handler_factory or DuplicateHandlerFactory()
        )
        self.default_dry_run = default_dry_run
        self.default_compression_level = default_compression_level
    ...
    def create(
        ...
        **kwargs,
        ...
        duplicate_strategy = (
            ...
            else self.default_duplicate_strategy
        )
        ...
        if processor_type == "standard":
            ...
            if not self.strategy_factory or not isinstance(
                self.strategy_factory, StrategyFactory
            ):
                self.strategy_factory = StrategyFactory(
                    default_dry_run=dry_run, default_compression_level=compression_level
                )
...
class ConfigBasedProcessorFactory(ProcessorFactory):
    ...
    def __init__(
        ...
        default_duplicate_strategy = self._get_config_value(
    ...
    def create_manga_processor(
        ...
        template_func: Optional[
            Callable[[Dict[str, Any], Optional[Callable]], str]
        ] = None,
        ...
        **kwargs,
        ...
        dry_run = (
            ...
            else self._get_config_value("dry_run", self.default_dry_run)
        )
        interactive = (
            interactive
            if interactive is not None
            else self._get_config_value("interactive", False)
        )

        # Create duplicate handler
        duplicate_handler = self.duplicate_handler_factory.create(
            strategy=self._get_config_value(
                "duplicate_strategy", self.default_duplicate_strategy
            ),
    ...
    def create_file_processor(
        ...
        **kwargs,
        ...
        return ResultFileProcessor(
            dry_run=dry_run,
            compression_level=self._get_config_value(
        ...
        **kwargs,
        ...
        if video_extensions is None:
            video_extensions = set(
                self._get_config_value(
                    "video.video_extensions", [".mp4", ".mkv", ".avi", ".mov"]
                )
            )
        ...
        if subtitle_extensions is None:
            subtitle_extensions = set(
                self._get_config_value(
                    "video.subtitle_extensions", [".srt", ".sub", ".idx", ".ass"]
                )
            )
    ...
    def create(self, processor_type: Optional[str] = None, **kwargs) -> P:
        """
        ...
        params["dry_run"] = kwargs.get(
            "dry_run", self._get_config_value("dry_run", self.default_dry_run)
        )
        params["compression_level"] = kwargs.get(
            "compression_level",
            self._get_config_value("compression_level", self.default_compression_level),
        )
        params["duplicate_strategy"] = kwargs.get(
            "duplicate_strategy",
            self._get_config_value(
                "duplicate_strategy", self.default_duplicate_strategy
            ),
        )
        params["duplicates_dir"] = kwargs.get(
            "duplicates_dir", self._get_config_value("duplicates_dir", None)
        )
        params["interactive"] = kwargs.get(
            "interactive", self._get_config_value("interactive", False)
        )
...
def create_processor_from_config(
    processor_type: Optional[str] = None, **kwargs
) -> Union[FileProcessor, ResultFileProcessor]:
    """
    Create a processor based on configuration.
    ...
    Returns:
        Created processor
    """
    return config_processor_factory.create(processor_type=processor_type, **kwargs)
