...
from collection_sorter.common.result import (
    Result, PathResult, FilesResult, BoolResult, OperationError, ErrorType, result_handler
)
    ...
    def process_file(
        ...
        **kwargs
        ...
        try:
            ...
            final_destination_result = self._handle_duplicates(source_path, destination_path)
            ...
            operation_result = self._execute_operation(source_path, final_destination, **kwargs)
    ...
    def _handle_duplicates(
        ...
        if self.duplicate_handler:
            try:
                ...
                if is_duplicate and self.duplicate_handler.strategy == DuplicateStrategy.SKIP:
            ...
            try:
                ...
                new_name = f"{destination_path.stem}_{timestamp}{destination_path.suffix}"
    ...
    def process_directory(
        ...
        **kwargs
        ...
        try:
            ...
            if destination:
                destination_path_result = self._prepare_destination_directory(destination)
                if destination_path_result.is_failure():
                    return destination_path_result
            ...
            # Step 3: Handle duplicates if needed
            final_destination_result = self._handle_directory_duplicates(source_path, destination_path)
            if final_destination_result.is_failure():
                return final_destination_result
            ...
            pre_result = self._pre_process_directory(source_path, final_destination, **kwargs)
            if pre_result.is_failure():
                return pre_result
            
            # Step 5: Execute the operation (to be implemented by subclasses)
            operation_result = self._execute_directory_operation(source_path, final_destination, **kwargs)
            if operation_result.is_failure():
                return operation_result
            ...
            if self.recursive:
                subdirs_result = self._process_subdirectories(source_path, processed_path, **kwargs)
                if subdirs_result.is_failure():
                    return subdirs_result
            
            # Step 7: Perform post-processing (optional)
            post_result = self._post_process_directory(source_path, processed_path, **kwargs)
            if post_result.is_failure():
                return post_result
    ...
    def _handle_directory_duplicates(
        ...
        if self.duplicate_handler and destination_path.exists:
            try:
                ...
                if self.duplicate_handler.strategy in [DuplicateStrategy.RENAME_NEW, DuplicateStrategy.RENAME_EXISTING]:
        ...
        try:
            ...
            for subdir in subdirs:
                ...
                if result.is_failure():
                    ...
                    logger.error(f"Failed to process subdirectory {subdir}: {result.error()}")
    ...
    def _execute_directory_operation(
        ...
        **kwargs
        ...
        try:
            ...
            if not destination_path.exists:
                if self.dry_run:
                    logger.info(f"Would move directory: {source_path} -> {destination_path}")
                    return Result.success(destination_path)
                else:
                    moved_path = source_path.move_to(destination_path)
                ...
                for root, dirs, files in os.walk(source_path.path):
                    ...
                    for file in files:
                        ...
                        if archive_name:
                            ...
                            rel_path = Path(archive_name) / file_path.relative_to(source_path.path)
                        else:
...
class BatchProcessorTemplate(ABC):
    ...
    def _prepare_batch_destination(
        ...
        if not destination_path.exists:
            ...
            raise ValueError(f"Destination exists but is not a directory: {destination_path}")
        ...
        try:
            ...
            if source.is_file:
                ...
                if self.directory_processor:
                    ...
                    return self.directory_processor.process_directory(source, dest_dir, **kwargs)
                else:
                    error = OperationError(
                        type=ErrorType.OPERATION_FAILED,
