...
class ConfigManager:
            ...
            if path.exists():
                try:
                    ...
                    raise ConfigurationError(
                        f"Error loading config file {path}: {str(e)}"
                    )
            else:
        ...
        try:
            with open(path, "r") as f:
                if suffix == ".yaml" or suffix == ".yml":
                    ...
                    if tomli is None:
                        raise ConfigurationError(
                            "tomli package not installed, cannot read TOML files"
                        )
    ...
    def get_command_config(self, command: str) -> Dict[str, Any]:
        ...
        if hasattr(self._config, command):
            ...
            command_dict = (
                command_config.model_dump()
                if hasattr(command_config, "model_dump")
                else {}
            )
    ...
    def generate_template(self, format: str = "yaml") -> str:
        ...
        if format.lower() == "yaml":
            ...
            if tomli is None:
                raise ValueError(
                    "tomli package not installed, cannot generate TOML template"
                )
            import toml
    ...
    def apply_click_context(self, ctx) -> None:
        ...
        if ctx.params:
            self.load_configuration(
                config_file=ctx.params.get("config"), cli_args=ctx.params
            )
