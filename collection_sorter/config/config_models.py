...
class CollectionConfig(BaseModel):
    ...
    archive: bool = Field(False, description="Create archives of processed files")
    move: bool = Field(False, description="Remove source files after processing")
    dry_run: bool = Field(
        False, description="Simulate operations without making changes"
    )
    ...
    def validate_duplicate_strategy(cls, v):
        ...
        if v.lower() not in valid_strategies:
            raise ValueError(
                f"Duplicate strategy must be one of: {', '.join(valid_strategies)}"
            )
        return v.lower()
...
class UIConfig(BaseModel):
    ...
    interactive: bool = Field(
        False, description="Enable interactive mode with confirmation prompts"
    )
    progress_bars: bool = Field(
        True, description="Show progress bars during operations"
    )
    color_output: bool = Field(True, description="Use colored terminal output")


class MangaConfig(BaseModel):
    """Manga-specific configuration."""

    author_folders: bool = Field(False, description="Organize manga by author")
    template: Optional[str] = Field(
        None, description="Template for manga folder naming"
    )
...
class RenameConfig(BaseModel):
    ...
    patterns: Dict[str, str] = Field(
        default_factory=dict, description="Regex patterns to use for renaming"
    )
...
class ZipConfig(BaseModel):
    ...
    nested: bool = Field(False, description="Create nested archives")
...
class AppConfig(BaseModel):
    ...
    ui: UIConfig = Field(
        default_factory=UIConfig, description="User interface configuration"
    )

    # Command-specific configurations
    manga: MangaConfig = Field(
        default_factory=MangaConfig, description="Manga-specific configuration"
    )
    video: VideoConfig = Field(
        default_factory=VideoConfig, description="Video-specific configuration"
    )
    rename: RenameConfig = Field(
        default_factory=RenameConfig, description="Rename-specific configuration"
    )
    zip: ZipConfig = Field(
        default_factory=ZipConfig, description="Archive-specific configuration"
    )
