...
def move_file(
    ...
    if not src_path.exists():
        raise FileOperationError(
            f"Source file does not exist: {src_path}", path=str(src_path)
        )
    ...
    try:
        ...
        if (
            duplicate_handler
            and duplicate_handler.strategy == DuplicateStrategy.MOVE_TO_DUPLICATES
            and duplicate_handler.duplicates_dir
        ):
...
def move_folder(
    ...
    if not src_path.exists():
        raise FileOperationError(
            f"Source folder does not exist: {src_path}", path=str(src_path)
        )
    ...
    if not dst_path.exists():
        try:
            ...
            raise FileOperationError(
                f"Failed to move folder: {str(e)}", path=str(src_path)
            )
    ...
    try:
        ...
        raise FileOperationError(
            f"Failed to move folder contents: {str(e)}", path=str(src_path)
        )
