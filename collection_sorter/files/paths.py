...
class FilePath:
        ...
        if create_if_missing and not self._path.exists():
            try:
                ...
                raise FileOperationError(
                    f"Failed to create path: {e}", path=str(self._path)
                )

        # Validate existence if required
        if must_exist and not self._path.exists():
            raise FileOperationError(
                f"Path does not exist: {self._path}", path=str(self._path)
            )
    ...
    def with_name(self, name: str) -> "FilePath":
        ...
        """
        return FilePath(self._path.with_name(name), must_exist=False)
    ...
    def with_suffix(self, suffix: str) -> "FilePath":
        ...
        """
        return FilePath(self._path.with_suffix(suffix), must_exist=False)
    ...
    def list_dir(self) -> List["FilePath"]:
        ...
        if not self.is_directory:
            raise FileOperationError(
                f"Not a directory: {self._path}", path=str(self._path)
            )
        ...
        try:
            ...
            raise FileOperationError(
                f"Failed to list directory: {e}", path=str(self._path)
            )
    ...
    def relative_to(self, other: Union[str, Path, "FilePath"]) -> Path:
        ...
        try:
            ...
            raise FileOperationError(
                f"Failed to compute relative path: {e}", path=str(self._path)
            )
