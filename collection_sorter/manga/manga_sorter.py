...
from collection_sorter.common.sorter import MultiThreadTask, BaseCollection
...
class MangaSorter(MultiThreadTask):
    ...
    def __init__(self, template=manga_template_function, archive=False, replace_function=None, parser=MangaParser, remove=False) -> None:
        super().__init__()
    ...
    def execute(self, source: Path, destination: Path):
        ...
        if directories:
            for directory in directories:
                ...
                new_name = self._template(updated, symbol_replace_function=self._replace_function)
                self._directory_action(name=new_name,
                                       collection=collection,
                                       destination=manga_destination,
                                       )
        else:
            new_name = self._template(manga_info, symbol_replace_function=self._replace_function)
            self._directory_action(name=new_name,
                                   collection=collection,
                                   destination=manga_destination,
                                   )

    def _directory_action(self, name: str, collection: MangaCollection, destination: Path):
        logger.info(f"Change collection: {collection}")

        try:
            if self._archive:
                archive = collection.archive_directory(destination=destination, new_name=name)
