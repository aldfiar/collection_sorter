...
def move_file(
    ...
    if (
        is_duplicate
        and duplicate_handler
        and duplicate_handler.strategy == DuplicateStrategy.SKIP
    ):
...
def rename_file(
    ...
    if isinstance(new_name, (str, Path)) and (
        "/" not in str(new_name) and "\\" not in str(new_name)
    ):
...
def archive_directory(
    ...
    if destination:
        dest_dir = FilePath(
            destination, PathType.DIRECTORY, create_if_missing=True
        ).path
    ...
    if archive_path.exists():
        if duplicate_handler:
            ...
            while final_archive_path.exists():
                ...
                final_archive_path = archive_path.with_name(
                    f"{stem}_{identifier}{suffix}"
                )
                counter += 1
                identifier = str(counter)
            is_duplicate = True
...
@result_handler
def delete_file(path: Union[str, Path, FilePath], dry_run: bool = False) -> Path:
    """
...
def delete_directory(
    path: Union[str, Path, FilePath], recursive: bool = True, dry_run: bool = False
) -> Path:
    """
...
def chain_operations(
    operations: List[Callable[..., PathResult]]
) -> Callable[..., PathResult]:
    """
...
def move_and_rename(
    ...
    move_result = move_file(
        source, destination, duplicate_handler=duplicate_handler, dry_run=dry_run
    )
    ...
    # Rename the file
    return rename_file(
        moved_path, new_name, duplicate_handler=duplicate_handler, dry_run=dry_run
    )
