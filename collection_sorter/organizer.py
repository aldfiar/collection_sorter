...
class CollectionSorter(MultiThreadTask):
    def __init__(self, template=sort_template, archive=False, replace_function=None, remove=False) -> None:
        super().__init__()
    ...
    def _to_destination(self, source: Path, destination: Path):
        ...
        if self._archive:
            if destination:
                archive = collection.archive_directory(destination=destination, new_name=root_name)
