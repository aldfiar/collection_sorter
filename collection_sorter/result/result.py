...
class Result(Generic[T, E], ABC):
    ...
    @staticmethod
    def from_exception(func: Callable[..., T], *args, **kwargs) -> Result[T, Exception]:
        """
...
class Failure(Result[Any, E]):
    ...
    def unwrap(self) -> Any:
        ...
        if isinstance(self._error, Exception):
            raise ResultError(
                f"Unwrapped failure result: {self._error}"
            ) from self._error
        else:
...
class OperationError:
    ...
    @staticmethod
    def from_exception(
        exception: Exception, path: Optional[Union[str, Path]] = None
    ) -> "OperationError":
        """
        ...
        elif isinstance(exception, IsADirectoryError) or isinstance(
        ...
        elif isinstance(exception, KeyboardInterrupt) or isinstance(
            exception, EOFError
        ):
