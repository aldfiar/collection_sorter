    ...
    def validate(self, value: Any) -> ValidationResult[Set[str]]:
        ...
        try:
                ...
                for ext in value:
                    if not isinstance(ext, str):
                        return ValidationResult.failure(
                            f"Invalid extension type: {type(ext)}"
                        )
                    extensions.add(ext if ext.startswith(".") else f".{ext}")
            else:
                return ValidationResult.failure(
                    f"Invalid extensions type: {type(value)}"
                )
            ...
            if self.valid_extensions and not extensions.issubset(self.valid_extensions):
                ...
                return ValidationResult.failure(
                    f"Invalid extensions: {', '.join(invalid)}"
                )
...
class BaseProcessorValidator:
    ...
    def validate_parameters(
        ...
        if "duplicate_handler" in kwargs:
            dup_result = self._validate_duplicate_handler(
                kwargs.get("duplicate_handler")
            )
            if not dup_result:
                errors.append(
                    OperationError(
    ...
    def _validate_destination(self, destination_path) -> ValidationResult[FilePath]:
        ...
        validator = PathValidator(
            must_exist=False, create_if_missing=True, must_be_dir=True
        )
        return validator.validate(destination_path)

    def _validate_duplicate_handler(
        self, duplicate_handler
    ) -> ValidationResult[DuplicateHandler]:
        """
        ...
        if not isinstance(duplicate_handler, DuplicateHandler):
            return ValidationResult.failure(
                "Duplicate handler must be an instance of DuplicateHandler"
            )
...
class BaseFileProcessor(BatchProcessorTemplate):
    ...
    def __init__(
        ...
        if validation_result.is_failure():
            ...
            self.destination_path = (
                FilePath(destination_path) if destination_path else None
            )
        else:
    ...
    def _collect_files(
        self, directory: FilePath, recursive: bool = True
    ) -> List[FilePath]:
        """
