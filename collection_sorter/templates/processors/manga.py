...
class MangaProcessorValidator(BaseProcessorValidator):
    ...
    def validate_parameters(
        ...
        if (
            ...
            and validated["destination_path"] is not None
        ):
            # Ensure destination directory can be created
            if not validated["destination_path"].exists and not kwargs.get(
                "dry_run", False
            ):
                try:
                    validated["destination_path"].path.mkdir(
    ...
    def _execute_implementation(self) -> Result[Dict[str, Any], List[OperationError]]:
        ...
        try:
            ...
            if (
                ...
                and not self.destination_path.exists
            ...
            manga_dirs = [
                FilePath(d) for d in self.source_path.path.iterdir() if d.is_dir()
            ]
        ...
        try:
            if self.archive:
                ...
                if not self.destination_path:
                    logger.warning(
                        "No destination specified for author folder processing"
                    )
                    return Result.success(self.stats)
                ...
                for manga_dir in manga_dirs:
                    ...
                    logger.info(
                        f"Archived manga directory {manga_dir} to {archive_path}"
                    )
                ...
                if self.move_source:
                    ...
                    logger.info(
                        f"Removed source directory after archiving: {self.source_path}"
                    )
                    self.stats["moved"] += 1
    ...
    def _process_batch_item(
        ...
        try:
            ...
            if self.archive:
                ...
                # Create the archive
                with ZipFile(
                    archive_path.path, "w", compression=ZIP_DEFLATED, compresslevel=6
                ) as zf:
                    # Add all files to the archive
