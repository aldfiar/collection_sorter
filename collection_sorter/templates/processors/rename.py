...
class PatternValidator(Validator):
    ...
    def validate(self, value: Any) -> ValidationResult[Dict[str, str]]:
        ...
        if not isinstance(value, dict):
            return ValidationResult.failure(f"Patterns must be a dictionary, got {type(value)}")
        ...
        for pattern, replacement in value.items():
            ...
            if not isinstance(replacement, str):
                errors.append(f"Replacement for '{pattern}' must be a string, got {type(replacement)}")
                continue
...
class RenameProcessorValidator(BaseProcessorValidator):
    ...
    def validate_parameters(self, **kwargs) -> Result[Dict[str, Any], List[OperationError]]:
        ...
        if validated.get('archive', False) and validated.get('move_source', False):
            logger.warning("Both archive and move_source are set to True - files will be archived and sources removed")
