...
class VideoProcessorValidator(BaseProcessorValidator):
    ...
    def validate_parameters(
        ...
        video_extensions_result = self._validate_video_extensions(
            kwargs.get("video_extensions")
        )
        ...
        subtitle_extensions_result = self._validate_subtitle_extensions(
            kwargs.get("subtitle_extensions")
        )
        ...
        if "source_path" in validated:
            if not (
                validated["source_path"].is_file
                or validated["source_path"].is_directory
            ):
                errors.append(
                    OperationError(
                        type=ErrorType.VALIDATION_ERROR,
                ...
                if not any(
                    str(validated["source_path"].path).lower().endswith(ext)
        ...
        if (
            "destination_path" in validated
            and not validated["destination_path"].exists
    ...
    def _validate_video_extensions(self, extensions) -> ValidationResult[Set[str]]:
        ...
        if result.is_valid and not any(
            ext.lower() in common_video_exts for ext in result.value
        ):
    ...
    def _validate_subtitle_extensions(self, extensions) -> ValidationResult[Set[str]]:
        ...
        # If no common extensions found, warn but accept
        if result.is_valid and not any(
            ext.lower() in common_subtitle_exts for ext in result.value
        ):
...
class VideoProcessorTemplate(BaseFileProcessor):
    ...
    def _parse_video_filename(self, filename: str) -> Dict[str, Any]:
        ...
        return {"title": title, "season": season, "episode": episode, "original": name}
