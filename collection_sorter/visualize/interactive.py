...
def select_destination(
    ...
    default = (
        ...
        else str(src_path.parent / "processed")
    )

    console.print(f"[bold]Source:[/bold] {src_path}")
    destination = prompt_input("Enter destination path", default=default)

    # Create destination if it doesn't exist
    dest_path = Path(destination)
    if not dest_path.exists():
        if confirm_action(
            f"Destination {dest_path} does not exist. Create it?", default=True
        ):
...
def interactive_preview(
    source_path: Union[str, Path], changes: Dict[str, str], operation: str = "rename"
) -> Dict[str, str]:
    """
    ...
    if not confirm_action(
        f"Proceed with {len(changes)} {operation} operations?", default=True
    ):
        return {}
...
def show_results(results: Dict[str, Any], operation: str = "Process") -> None:
    """
